...
public class TopBtn{
	...
	//トグルボタンx,yはメートル換算
	public TopBtn(CCLayer layer, int ptmRatio,
	...
	private void makeToggleBtn(float x, float y, float size){
		...
		/////////////////////
        //ボタンのスプライトシート
		CCSpriteSheet ssBack = CCSpriteSheet.spriteSheet("btn_top.png", 100);
		layer.addChild(ssBack, 1, BACK_BTN);
		
		CGRect rect = CGRect.make(0, 0, 160, 50);
		...
		label.setPosition(x * ptmRatio, y * ptmRatio);
	...
	public void on(){
		if(!active){
			active = true;
			CGRect activeRect = CGRect.make(0, 50, 160, 50);
	...
	public void off(){
		if(active){
			active = false;
			CGRect inactiveRect = CGRect.make(0, 0, 160, 50);
